...
contract TriangularArbitrage {
    ...
    address public WCFX_ADDRESS = 0x8d7DF9316FAa0586e175B5e6D03c6bda76E3d950;
    address public CUSDT_ADDRESS = 0x8b8689C7F3014A4D86e4d1D0daAf74A47f5E0f27;
    address public TREA_ADDRESS = 0x83F924c12144807Da960DDb8934256F9aCA14408;
    address public CUSDT_WCFX_ADDRESS = 0x8D545118D91C027C805c552f63A5c00a20aE6Aca;
    address public CUSDT_TREA_ADDRESS = 0x8b33f372AA0b7F83f4dE77218d00FDEB17366c0a;
    address public TREA_CFX_ADDRESS = 0x855e50A759ec0074aBda7B49E15Bf4393462C571;
    ...
    //设置wcfx地址
    function setWCFXAddress(address _address) public onlyOwner {
        WCFX_ADDRESS = _address;
    } 
    //设置cusdt地址
    function setCUSDTAddress(address _address) public onlyOwner {
        CUSDT_ADDRESS = _address;
    } 
    //设置trea地址
    function setTREAAddress(address _address) public onlyOwner {
        TREA_ADDRESS = _address;
    } 
    ...
    function FluxToUsdtPrice(uint256 cFlux) public view returns (uint256) { 
        (uint112 reserve0, uint112 reserve1, uint32 blockreTimestampLast) = IUniswapV2Pair(address(0x805E1F3E0B9500CC00a99915aE831d1D927737Ae)).getReserves(); 
        return cFlux * reserve0 / reserve1; 
    }

    //三角套利范例：wcfx-cusdt-trea-wcfx, _tokenAddress = wcfx
    //反复触发这个函数。检查套利空间
    function doTriangularArbitrage(address _tokenAddress) public onlyOwner returns (uint256) { 
        //wcfx-cusdt-trea-wcfx
        if(_tokenAddress == address(0x8d7DF9316FAa0586e175B5e6D03c6bda76E3d950)){ 
            uint256 benefit = 0; //利润
            uint256 amount = 0; //初始兑换量
            (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast0) = IUniswapV2Pair(CUSDT_WCFX_ADDRESS).getReserves(); 
            (uint112 reserve2, uint112 reserve3, uint32 blockTimestampLast1) = IUniswapV2Pair(CUSDT_TREA_ADDRESS).getReserves(); 
            (uint112 reserve4, uint112 reserve5, uint32 blockTimestampLast2) = IUniswapV2Pair(TREA_CFX_ADDRESS).getReserves(); 

            uint256 currentRate1 = reserve0 / reserve1;
            uint256 currentRate2 = reserve3 / reserve2;
            uint256 currentRate3 = reserve5 / reserve4;

            uint256 totalRate = currentRate1 * currentRate2 * currentRate3;

            if(totalRate > 1 ){
                //大于一说明有利可图,进行三角兑换
                amount = 1000000000000000000;//这里有一个计算公式，先换1个
                //兑换函数，不确定参数是不是这么给。 swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external;
                uint256 swapResulat1 = IUniswapV2Pair(CUSDT_WCFX_ADDRESS).Swap(0,amount,msg.sender,"");
                uint256 swapResulat2 = IUniswapV2Pair(CUSDT_TREA_ADDRESS).Swap(swapResulat1,0,msg.sender,"");
                uint256 swapResulat3 = IUniswapV2Pair(TREA_CFX_ADDRESS).Swap(swapResulat2,0,msg.sender,"");
                benefit = swapResulat3 - amount;
            }
            return benefit; 
        }
    }
